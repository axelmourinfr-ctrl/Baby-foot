...
async function loadFromIDB() {
  try {
    ...
    Object.entries(all).forEach(([k, entry]) => {
      ...
      // Ne jamais écraser une valeur localStorage plus récente que la copie IndexedDB
      if(localTs > idbTs) {
        // localStorage est déjà plus frais (l'écriture IDB précédente n'avait pas eu le temps de se terminer) :
        // on garde localStorage tel quel et on remet _cache en phase avec lui, pas avec l'ancienne copie IDB.
        try {
          const localRaw = localStorage.getItem('bfc_'+k);
          if(localRaw !== null) _cache[k] = JSON.parse(localRaw);
        } catch(e) { _cache[k] = entry.v; }
        return;
      }
      _cache[k] = entry.v;
      try {
    ...
    console.log('IDB loaded:', Object.keys(all).length, 'keys, profile:', !!_cache['profile']);
