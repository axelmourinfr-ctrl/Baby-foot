...
async function authSignIn() {
  ...
  if(error) { showAuthMsg('Erreur : '+error.message, 'error'); return; }
  ...
  if(loaded) {
    ...
    SAVE_KEYS.forEach(k => { const v = DB.get(k); if(v) CloudDB.sync(k, v); });
...
async function authSignUp() {
  ...
  if(error) { showAuthMsg('Erreur : '+error.message, 'error'); return; }
  showAuthMsg('Compte créé ! Vérifie ton email puis connecte-toi.', 'status');
